      ...
      >
        {data?.heroBannerCollection.items.map((item, index) => (
          <SwiperSlide className='' key={item.heroBannerTitle + index}>
            <Image className='w-full max-h-screen object-cover' src={item.heroBannerImage.url} width={1920} height={740}
                   quality={50} alt={item.heroBannerTitle}/>
            <div className='absolute w-full h-full inset-0 bg-black/60'></div>
            <h2
              className='absolute left-[20%] top-[30%] text-4xl sm:text-5xl lg:text-6xl font-semibold text-white sm:w-8/12'>
              {item.heroBannerTitle}
            </h2>
            <h3
              className='absolute left-[20%] top-[39%] text-2xl sm:text-3xl lg:text-4xl font-semibold text-white sm:w-8/12'>
              {item.subTitle}
            </h3>
            <h3
